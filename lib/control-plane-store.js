...
import { getAtlasDashboard } from './dashboard.js';
...
export async function todayAndRecord(input = {}) {
  const dashboard = await getAtlasDashboard(input);
  const plan = dashboard.today;
  const date = new Date(input.now || Date.now());
  const planDate = Number.isFinite(date.getTime()) ? date.toISOString().slice(0, 10) : new Date().toISOString().slice(0, 10);
  const activeProjectId = input.active_project_id || null;
  const { rows } = await pool.query(`
  ...
  return { ...plan, daily_plan_id: rows[0].id, persisted_at: rows[0].updated_at || rows[0].created_at };
}
