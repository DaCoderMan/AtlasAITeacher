...
export function legacyBearerEnabled() {
  const configured = Boolean(env('ATLAS_MCP_SECRET'));
  if (!configured) return false;
  if (!publicHostedRuntime()) return true;
  return process.env.ATLAS_MCP_ALLOW_LEGACY_BEARER_PUBLIC === 'true';
}
...
export function anyMcpAuthModeEnabled() {
  return tunnelUnauthenticatedEnabled() || legacyBearerEnabled() || oauthEnabled();
}
...
export async function authenticateMcpRequest(req) {
  ...
  if (!anyMcpAuthModeEnabled()) {
    ...
    const unsafePublicLegacyBearer = Boolean(env('ATLAS_MCP_SECRET')) && publicHostedRuntime();
    return {
      ok: false,
      status: 503,
      reason: unsafePublicTunnel
        ? 'atlas_mcp_public_unauthenticated_forbidden'
        : unsafePublicLegacyBearer
          ? 'atlas_mcp_public_legacy_bearer_forbidden'
          : 'atlas_mcp_auth_not_configured'
    };
  }
  ...
  const legacy = legacyBearerEnabled() ? env('ATLAS_MCP_SECRET') : null;
