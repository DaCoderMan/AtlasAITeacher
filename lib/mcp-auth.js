...
export function anyMcpAuthModeEnabled() {
  return process.env.ATLAS_MCP_ALLOW_UNAUTHENTICATED === 'true' || Boolean(env('ATLAS_MCP_SECRET')) || oauthEnabled();
}
...
export function protectedResourceMetadata(req) {
  const authorizationServer = env('ATLAS_MCP_OAUTH_ISSUER');
  return {
...
export async function authenticateMcpRequest(req) {
  if (process.env.ATLAS_MCP_ALLOW_UNAUTHENTICATED === 'true') {
    return { ok: true, mode: 'tunnel', subject: 'tunnel', scopes: ['atlas.read', 'atlas.write'] };
  }

  if (!anyMcpAuthModeEnabled()) return { ok: false, status: 503, reason: 'atlas_mcp_auth_not_configured' };
