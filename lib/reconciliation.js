...
export async function reconcileAtlas({ staleSourceMinutes = 180, repair = true } = {}) {
  ...
  try {
    ...
    const result = { run_id: runId, status, ...summary };
    await recordMutationJournal({
      operation: 'atlas_reconcile',
      contentText: `Completed Atlas reconciliation run ${runId} with status ${status}`,
      contentJson: result,
      rollbackNote: 'Delete the reconciliation run record if it was persisted for the wrong scope or input.'
    });
    return result;
  } catch (error) {
    await pool.query(`
