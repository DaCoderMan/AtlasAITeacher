...
function scoreTask(task, context) {
  ...
  if (project.id) evidence.push({ type: 'project', id: project.id, field: 'priority', value: project.priority ?? null });
  ...
  const waiting = task.status === 'waiting' || Boolean(task.waiting_on);
  ...
  const proposedMajorBuild = ['Building', 'Testing'].includes(project.status) && task.starts_major_build;
  ...
  const weighted =
    components.urgency * 0.24 +
    components.task_priority * 0.18 +
    components.project_priority * 0.14 +
    components.strategic_impact * 0.16 +
    components.income_impact * 0.10 +
    components.blocker_release * 0.08 +
    components.dependency_value * 0.06 +
    components.calendar_fit * 0.04 +
    components.waiting_penalty +
    components.context_switch +
    components.wip_penalty;
...
export function buildTodayPlan({ tasks = [], projects = [], commitments = [], now = new Date().toISOString(), active_project_id = null, max_major_wip = 3 } = {}) {
  ...
  const majorWipCount = projects.filter(project => ['Building', 'Testing'].includes(project.status)).length;
  ...
  const warnings = [];
  if (majorWipCount >= max_major_wip) warnings.push({ type: 'wip_limit_reached', active_major_projects: majorWipCount, max_major_wip });
  ...
  return {
    ...
    warnings,
    wip: { major_building_or_testing: majorWipCount, max_major_wip }
  };
}
