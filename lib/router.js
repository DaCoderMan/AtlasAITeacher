...
export function routeAgent({ resolved_context, intent = '', mode = null, risk = 'low', language = null, service_health = [] } = {}) {
  ...
  let rule = INTENT_RULES.find(candidate => candidate.pattern.test(text));

  if (manifest?.active_agent && (!rule || !manifest.allowed_agents.includes(rule.agent))) {
    rule = { agent: manifest.active_agent, workflow: mode || 'project' };
  }
  if (!rule) rule = { agent: 'Atlas', workflow: mode || 'orchestrate' };
  ...
  const qa_required = Boolean(manifest?.qa_required || agent.qa_default || ['medium', 'high'].includes(risk) || rule.workflow === 'build');
  return {
    ...
    risk,
    rationale: {
      rule: rule.agent === agent.name ? 'deterministic_intent_or_project_rule' : 'safe_fallback',
