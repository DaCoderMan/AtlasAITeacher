import pg from 'pg';
import { classifyConnectorRuntimeState } from './connector-state.js';
import { connectorTestPlan } from './connector-tests.js';

const { Pool } = pg;
const pool = new Pool({ connectionString: process.env.DATABASE_URL, max: 2 });

const SCHEMA_VERSION = '2026-08-15.cx003';
const SCHEMA_COMPATIBILITY = '2026-08-15.cx004';
...
function currentSchemaLineage(connector) {
  return {
    current_version: SCHEMA_VERSION,
    compatibility_epoch: SCHEMA_COMPATIBILITY,
    observed_version: SCHEMA_VERSION,
    status: 'current'
  };
}

function schemaLineageForRecord(connector, row = {}) {
  const currentVersion = SCHEMA_VERSION;
  const observedVersion = row.schema_version || currentVersion;
  let status = 'current';
  if (!row.schema_version) status = 'derived';
  else if (row.schema_version !== currentVersion) status = 'stale';
  return {
...
function defaultConnectorRecord(connector, env = process.env) {
  ...
  return {
    ...
    schema_lineage: currentSchemaLineage(connector),
    metadata: { derived_from_runtime: true },
    test_plan: connectorTestPlan(connector.id)
  };
}
...
function mergeConnectorRecord(base, row = {}) {
  return {
    ...
    schema_lineage: schemaLineageForRecord(base, row),
    metadata: row.metadata || base.metadata,
    state: row.state || base.state,
    test_plan: row.test_plan || base.test_plan,
    updated_at: row.updated_at || null
  };
}

function shouldFallbackToRuntimeDefaults(error) {
  return ['42P01', 'ECONNREFUSED', 'ENOTFOUND'].includes(error?.code);
}
  ...
  try {
    ...
    for (const row of installations.rows) {
      const base = connectors.get(row.connector_id) || {
        ...
        last_success_at: null,
        last_error: null,
        schema_lineage: schemaLineageForRecord(row, row),
    ...
    for (const row of sources.rows) {
      ...
      connectors.set(row.source, {
        ...
        state: classifyConnectorRuntimeState({
    ...
    if (!shouldFallbackToRuntimeDefaults(error)) throw error;
    for (const connectorId of connectors.keys()) {
      ...
      connectors.set(connectorId, {
        ...
        metadata: {
          ...(current.metadata || {}),
          registry_fallback_reason: error.code
        }
      });
    }
  }

  const rows = [...connectors.values()]
    .map(row => ({ ...row, state: row.state || classifyConnectorRuntimeState(row) }))
