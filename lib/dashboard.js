...
import { atlasConnectors } from './connector-registry.js';
...
export async function getAtlasDashboard({ now = new Date().toISOString(), active_project_id = null } = {}) {
  const [projects, tasks, automation, system_health, controlPlane, connectors, executionRuns, executionRunEvents] = await Promise.all([
    ...
    atlasConnectors(),
    listExecutionRuns({ active_only: true, limit: 10 }),
    listExecutionRunEvents({ limit: 20 })
  ]);
  ...
  return {
    ...
    daily_brief: {
      ...
      active_execution_runs: executionRuns.length,
    ...
    quality_gate: getQualityGateStatus(),
    execution_runs: {
      active: executionRuns,
      recent_events: executionRunEvents
    },
    system_health,
