...
const CANONICAL_ALIASES = {
  'atlas — life os & ai teacher': 'atlas',
  'magic cloud storage solution / agent memory infrastructure': 'magic-cloud-storage',
  'magic hebrew learning system': 'magic-hebrew',
  'magic video pipeline': 'magic-video',
  'workitu tech growth & sales engine': 'workitu-growth',
  'remote tech job search': 'career'
};
...
export function validateManifest(manifest, { lifecycle = rawConfig.lifecycle } = {}) {
  ...
  for (const field of ['id', 'slug', 'name', 'mission', 'lifecycle', 'owner', 'active_agent', 'memory_namespace', 'autonomy']) assertString(manifest[field], field);
  ...
  if (!manifest.allowed_agents.includes(manifest.active_agent)) throw new Error(`active_agent ${manifest.active_agent} is not allowed by project ${manifest.id}`);
  return manifest;
}
...
export function getProjectManifest(value) {
  ...
  const raw = typeof value === 'object' ? value.manifest_id || value.id || value.slug || value.name : value;
  if (!raw) return null;
  const key = String(raw).trim().toLowerCase();
  const aliasId = CANONICAL_ALIASES[key];
  return registry.byId.get(key) || registry.bySlug.get(key) || registry.byName.get(key) || (aliasId ? registry.byId.get(aliasId) : null) || null;
}

export function requireProjectManifest(value) {
  const manifest = getProjectManifest(value);
  if (!manifest) throw new Error(`unknown Atlas project: ${typeof value === 'object' ? value?.name || value?.id : value}`);
  return manifest;
}
