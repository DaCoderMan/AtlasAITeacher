...
export async function atlasUpdateTask({
  ...
  scheduled_start, scheduled_end, blocker
} = {}) {
  ...
  await ingestEvent({
    source: 'atlas_mcp',
    source_event_id: `task-update:${task_id}:${Date.now()}`,
    content_text: `Updated task: ${rows[0].title}. Status ${rows[0].status}.`,
    content_json: rows[0], actor: 'atlas', project_hint: rows[0].project_id
  });
  return rows[0];
}
