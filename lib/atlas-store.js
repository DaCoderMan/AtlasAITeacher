...
export async function atlasSearch({ query, limit = 20 } = {}) {
  ...
  try {
    const [projects, tasks, extractions] = await Promise.all([
      client.query(`
        SELECT id::text, name, objective, status, priority, next_action, blockers, updated_at
        FROM projects
        WHERE user_id=$1 AND deleted_at IS NULL
          AND (name ILIKE $2 OR COALESCE(objective,'') ILIKE $2 OR COALESCE(next_action,'') ILIKE $2)
...
export async function atlasProjects({ status, limit = 50 } = {}) {
  ...
  const { rows } = await pool.query(`
    SELECT id::text, name, objective, status, priority, next_action, blockers, external_links, updated_at
    FROM projects
...
export async function atlasContext({ project, query, limit = 20 } = {}) {
  ...
  if (project) {
    params.push(`%${project}%`);
    projectFilter = ` AND name ILIKE $${params.length}`;
  }
...
export async function atlasStatus() {
  const [projects, tasks, routing] = await Promise.all([
  ...
  const top = await pool.query(`
    SELECT id::text, title, priority, due_at, status
  ...
  `, [userId()]);
  return { projects: projects.rows, tasks: tasks.rows, routing: routing.rows, top_tasks: top.rows };
}
...
export async function atlasCreateTask({ title, description, project_id, priority = 3, due_at } = {}) {
  ...
  await ingestEvent({ source: 'atlas_mcp', source_event_id: `task:${rows[0].id}`, content_text: `Created task: ${rows[0].title}`, content_json: rows[0], actor: 'codex' });
  return rows[0];
}

export async function atlasUpdateProject({ project_id, status, priority, next_action, blockers, objective } = {}) {
  ...
  const { rows } = await pool.query(`
    UPDATE projects SET
      ...
      blockers=CASE WHEN $7::boolean THEN $8 ELSE blockers END,
      objective=CASE WHEN $9::boolean THEN $10 ELSE objective END,
      updated_at=now()
    WHERE id=$2::uuid AND user_id=$1 AND deleted_at IS NULL
    RETURNING id::text, name, objective, status, priority, next_action, blockers, updated_at
  `, [userId(), project_id, status || null, priority == null ? null : Math.max(1, Math.min(5, Number(priority))),
  ...
  await ingestEvent({ source: 'atlas_mcp', source_event_id: `project-update:${project_id}:${Date.now()}`, content_text: `Project update: ${rows[0].name}. Status ${rows[0].status}. Next action: ${rows[0].next_action || 'none'}`, content_json: rows[0], actor: 'codex', project_hint: rows[0].name });
  return rows[0];
}

export async function atlasRemember({ text, project_hint, sensitivity = 'normal', source = 'atlas_mcp' } = {}) {
  if (!text?.trim()) throw new Error('text is required');
  return ingestEvent({ source, content_text: text, project_hint: project_hint || null, sensitivity, actor: 'codex' });
}
