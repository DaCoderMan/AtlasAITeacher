...
export function classifyEvent(input) {
  ...
  const add = (kind, title, importance, confidence, structured = {}) => {
    extractions.push({
      ...
      confidence,
      structured,
      canonical_key: `${kind}:${hashContent(`${title || ''}|${text}`)}`
    });
  };

  if (/\b(todo|to do|need to|must|remember to|remind me|next action|task)\b/i.test(text)) {
    add('task', text.slice(0, 160), 75, 0.78);
  }
  if (/\b(decided|decision|we will|we'll use|chosen|canonical|rule:)\b/i.test(text)) {
    add('decision', text.slice(0, 160), 85, 0.82);
  }
  if (/\b(project|repo|prd|module|architecture|implementation|milestone)\b/i.test(text)) {
    add('project_update', text.slice(0, 160), 70, 0.72, { project_hint: input.project_hint || null });
  }
  if (/\b(appointment|meeting|dentist|doctor|tomorrow|sunday|monday|tuesday|wednesday|thursday|friday|saturday|at \d{1,2}(:\d{2})?)\b/i.test(lower)) {
    add('commitment_candidate', text.slice(0, 160), 80, 0.64);
  }
  if (/\b(client|customer|lead|prospect|contact|phone|email|whatsapp)\b/i.test(lower)) {
    add('contact_or_sales', text.slice(0, 160), 65, 0.66);
  }
  if (/\b(learned|studied|lesson|course|progress|mastered|practice)\b/i.test(lower)) {
    add('learning_progress', text.slice(0, 160), 60, 0.68);
  }
  if (/\b(prefer|from now on|always|never|my goal|i want atlas|remember that)\b/i.test(lower)) {
    add('memory_candidate', text.slice(0, 160), 70, 0.72);
  }

  const importance = Math.max(10, ...extractions.map(x => x.importance));
  return { importance, confidence: extractions.length ? 0.74 : 0.5, extractions };
}
...
export async function ingestEvent(input) {
  ...
  const text = normalizeText(input.content_text || input.text || '');
  const contentHash = hashContent(text || JSON.stringify(input.content_json || {}));
  ...
  try {
    ...
    const inserted = await client.query(`
      ...
      DO UPDATE SET updated_at=now(), content_text=EXCLUDED.content_text,
    ...
    `, [
      ...
      input.content_type || 'text', text || null, JSON.stringify(input.content_json || {}),
    ...
    for (const extraction of classified.extractions) {
      const exists = await client.query(
        'SELECT id::text FROM atlas_extractions WHERE user_id=$1 AND canonical_key=$2 LIMIT 1',
    ...
    return { ok: true, event_id: eventId, importance: classified.importance, extractions: created };
  } catch (error) {
