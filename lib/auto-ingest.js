...
export function extractCursorCheckpoint(event = {}) {
  const provenance = event?.provenance && typeof event.provenance === 'object' ? event.provenance : {};
  const cursor = provenance.cursor || provenance.next_cursor || event.cursor || event.next_cursor || null;
  if (cursor == null || cursor === '') return null;
  return {
    connector: String(event.source || ''),
    cursor: String(cursor),
    state: provenance.cursor_state && typeof provenance.cursor_state === 'object'
      ? provenance.cursor_state
      : (event.cursor_state && typeof event.cursor_state === 'object' ? event.cursor_state : {})
  };
}
...
async function persistCursorCheckpoint(event = {}) {
  const checkpoint = extractCursorCheckpoint(event);
  if (!checkpoint?.connector) return null;
  await pool.query(`
    INSERT INTO atlas_connector_cursors(user_id, connector, cursor, state, updated_at)
    VALUES ($1,$2,$3,$4::jsonb,now())
    ON CONFLICT (user_id, connector)
    DO UPDATE SET cursor=EXCLUDED.cursor, state=EXCLUDED.state, updated_at=now()
  `, [userId(), checkpoint.connector, checkpoint.cursor, JSON.stringify(checkpoint.state || {})]);
  return checkpoint;
}
...
export async function processQueuedEvents({ limit = 25, stale_minutes = 10 } = {}) {
  ...
  for (const item of claimed) {
    try {
      ...
      const cursorCheckpoint = await persistCursorCheckpoint(item.payload);
      const policy = [];
      ...
      `, [userId(), item.id, result.event_id, JSON.stringify({ policy, routing, result, cursor_checkpoint: cursorCheckpoint })]);
      await markQueue(item.id, 'done');
      await touchSource(item.source, { success: true });
      results.push({ queue_id: item.id, ok: true, event_id: result.event_id, policy, routing, cursor_checkpoint: cursorCheckpoint });
    } catch (error) {
...
export async function getAutomationStatus() {
  const [queue, sources, routing, stuck, cursors] = await Promise.all([
    pool.query(`
    ...
    `, [userId()]),
    pool.query(`
      SELECT connector, cursor, state, updated_at
      FROM atlas_connector_cursors
      WHERE user_id=$1
  ...
  return {
    ...
    sources: sources.rows.map(source => {
      const cursor = cursors.rows.find(item => item.connector === source.source) || null;
      return {
        ...source,
        cursor: cursor ? {
          value: cursor.cursor,
