...
export async function requeueDeferredRoutes({ include_failed = false, limit = 100 } = {}) {
  const statuses = include_failed ? ['waiting_connector', 'failed'] : ['waiting_connector'];
...
export async function processPendingRoutes({ eventId = null, limit = 100 } = {}) {
  ...
  for (const route of rows) {
    ...
    try {
      ...
      await markRoute(route.id, 'failed', { automated: true, policy, error: message, failed_at: new Date().toISOString() });
      results.push({ route_id: route.id, destination, status: 'failed', error: message });
    }
  }
