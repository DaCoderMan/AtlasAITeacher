...
export async function processPendingRoutes({ eventId = null, limit = 100 } = {}) {
  ...
  for (const route of rows) {
    ...
    if (policy.decision === 'persist_private') {
      await markRoute(route.id, 'review', {
        automated: true,
        policy,
        review_reason: 'sensitive_content_requires_local_only'
      });
      await recordMutationJournal({
        operation: 'atlas_route_status_update',
        contentText: `Held route ${route.id} for local-only privacy review`,
        contentJson: {
          route_id: route.id,
          destination,
          status: 'review',
        ...
        projectHint: route.project_hint || null,
        rollbackNote: 'Restore the route status to pending only after explicit privacy policy allows external delivery.'
      });
    ...
    try {
      ...
      results.push({ route_id: route.id, destination, status: 'done', destination_ref: ref, destination_checksum: destinationChecksum });
    } catch (error) {
      ...
      results.push({ route_id: route.id, destination, status: classified.status, error: message, retryable: classified.retryable, destination_checksum: destinationChecksum });
    }
  }
