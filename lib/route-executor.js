...
export async function processPendingRoutes({ eventId = null, limit = 100 } = {}) {
  ...
  for (const route of rows) {
    ...
    try {
      ...
      await markRoute(route.id, 'done', { automated: true, policy, delivery: delivered }, ref ? String(ref) : null);
      results.push({ route_id: route.id, destination, status: 'done', destination_ref: ref });
    } catch (error) {
      await markRoute(route.id, 'failed', { automated: true, policy, error: String(error?.message || error) });
