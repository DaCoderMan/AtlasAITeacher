...
import { stageSyncDelivery, markSyncAttempt, markSyncDelivered, markSyncFailed } from './sync-ledger.js';
...
export async function processPendingRoutes({ eventId = null, limit = 100, syncMode = 'normal' } = {}) {
  ...
  for (const route of rows) {
    ...
    if (!url) {
      await markRoute(route.id, 'waiting_connector', { automated: true, policy, missing_env: envName || null });
    ...
    try {
      delivery = await stageSyncDelivery(pool, syncRoute, payload, {
        userId: userId(),
        mode: syncMode,
        provenance: { source: 'atlas_route_executor', event_provenance: route.provenance || {} }
      });
    } catch (error) {
    ...
    // A completed external delivery must not be emitted a second time merely because
    // a route was manually reset/replayed. Readback verification is tracked separately.
    ...
    try {
      ...
      await markRoute(route.id, 'done', {
        ...
        delivery: delivered,
        sync_delivery_id: delivery.id,
        sync_status: 'delivered_unverified'
      }, ref ? String(ref) : null);
      results.push({
        route_id: route.id,
        destination,
